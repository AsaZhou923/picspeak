...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description='Export the PicSpeak stage A analytics snapshot to Markdown.')
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f'Default: {dated_analytics_report_path(repo_root, today, "stage-a-baseline-snapshot")}'
        ),
    )
...
def main() -> None:
    ...
    if snapshot.get('generation_note'):
        markdown = (
            f"> 注：{snapshot['generation_note']}\n\n"
            + markdown
        )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(markdown, encoding='utf-8')
    print(f'Wrote analytics snapshot to {output_path}')
