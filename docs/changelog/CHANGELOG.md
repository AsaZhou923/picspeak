...
<a id="2026-09-26-gallery-and-critique-clarity"></a>

## 2026-09-26 - 准备中：长廊先看作品，点评说明更清楚

日期：2026-09-26

### 这次更新

这轮界面调整正在准备中：长廊会更快展示公开作品，点评结果与下一步操作也会更容易看懂。

- 进入长廊后会先看到照片；作品卡片减少装饰，保留作者、评分和打开点评的清晰入口。
- 公开点评示例的介绍会更简短；手机上会直接显示最需要改进的评分依据。
- 导航、筛选和操作控件会更方便使用键盘与触屏，小号文字也会更容易辨认。
- 长廊加载失败会与“暂时没有作品”分开提示，并提供重试入口。

---

<a id="2026-09-25-gallery-actions-and-clearer-controls"></a>

## 2026-09-25 - 加入长廊更直观，按钮含义更清楚

日期：2026-09-25

### 这次更新

看完点评后的公开展示、分享和保存入口更容易找到，每个按钮的作用与当前状态也更清楚。

- “加入长廊”直接显示在照片结果旁，历史页也能直接确认加入；从长廊打开自己的作品时，管理入口仍然可见。
- 未加入、等待审核、已公开和审核未通过会分别显示对应状态与操作，不再出现“未展示”却只有不可用的“移出”按钮。
- 长廊展示与分享链接可以分别关闭；创建、复制、查看和关闭链接的入口按当前状态显示，公开范围更容易理解。
- 中英日按钮名称和提示更加一致；生成按钮不可用时说明原因，上传支持键盘操作和失败后重试，深色界面的辅助文字更易阅读。

这次调整不会自动公开任何照片。加入长廊仍需你主动确认，并在审核通过后展示；移出长廊不会关闭已有的分享链接。

---

<a id="2026-09-24-critique-organization-and-sharing"></a>

## 2026-09-24 - 点评历史、分享导出与练习体验更新

日期：2026-09-24

### 这次更新

点评和历史记录更容易找到，分享链接正常展示照片与点评，导出保留完整内容；练习从选原片、保存目标到查看每次比较结果连接起来。

### 分享、导出与结果页

- 分享链接打开照片和点评页面，之前生成的旧链接也能正常跳转；撤销后明确显示链接已失效。
- 图片卡自动生成预览，可直接保存 PNG，较长的点评也能完整展示。打印报告和 Markdown 包含优点、问题、改进建议与五维评分，不包含私人标签或备注。
- 结果页先展示照片、评分和主要问题；生成参考图、拍摄信息、备注与分享设置默认收起，分享和导出保留快捷入口。

### 历史、修改版比较与练习记录

- 历史页先显示点评或练习记录，筛选更紧凑，整理、分享、统计与推广内容默认收起。
- 修改版比较需要先在照片编辑器中修改，再上传新文件与原片对比；重新拍摄也使用新照片，不再默认拿同一张原图重复点评。
- 练习入口已恢复。可以从已有点评选原片，使用建议、模板或自行填写目标，保存后再上传修改版或重拍照片；还没有点评时会先引导完成第一张评图。
- 保存的目标、每次尝试和比较结果归入同一条练习记录。结果页可以直接返回这次练习，练习页可以回看结果或继续下一次尝试。

### 点评后的整理与浏览

私人标签、备注和搜索用于整理自己的点评；图库支持列数、紧凑模式、缩放、上下篇切换和近期作品。分享链接可独立撤销，公开主页可自行选择展示内容。

---

<a id="2026-09-23-critique-practice-and-sharing"></a>

## 2026-09-23 - 准备中：点评后的整理、复拍与分享

日期：2026-09-23

### 这次更新

这轮更新仍把上传评图放在第一入口，并把点评后的整理、复拍和分享动作接到同一条路径里；当前状态为准备中。

- 上传照片开始评图仍是首要动作；看完点评后，可以保存一个练习目标，并用下一张复拍结果对照证据。
- 私人标签、备注和练习日志档案会放在一起，方便找回某次点评、目标和尝试记录。
- 图库会加入列数、紧凑模式、缩放、前后篇切换和近期作品入口，方便连续浏览作品。
- 分享可单独撤销；公开主页可自行选择展示内容；图片卡和打印摘要用于保存或交给别人看。

---

<a id="2026-09-14-multilingual-experience-consistency"></a>
...
语言选择现在会更完整地贯穿页面、错误提示、创作与支付流程，减少使用过程中突然混入其他语言的情况。

- 首次访问会参考浏览器语言，切换后的语言会继续用于首页、长廊、更新记录、提示词案例和公开说明页面。
- 上传、点评、任务失败和生成失败会显示当前语言的说明，并保留请求编号，方便需要时联系支持。
- 照片历史、生成详情、激活码、按钮说明和读屏标签补齐中英日文案。
- 提示词案例会标明原始语言或本地化适配版本；生成和结账流程也会继续使用当前语言。

---

<a id="2026-09-08-evidence-based-scoring"></a>

## 2026-09-08 - 高分更有依据，进步对比更清楚

日期：2026-09-08

### 这次更新

高分更重视照片本身的完成度，前后进步的比较也更清楚。

- 高分判断会再次核对构图、光线和表达的实际效果，同时保留成立的剪影、留白、黑白等风格。
- 进步统计按相同评分标准比较，不把不同标准的旧分数直接混算。
- 新标准的评价标签与实际分档一致，7 分段不会提前显示成 8 分段。
- 已保存的点评和长廊作品分数保持原状，本次不批量重评。

---

<a id="2026-09-06-gallery-free-reassessment"></a>

## 2026-09-06 - 长廊作品已完成免费重评

日期：2026-09-06

### 这次更新

影像长廊的 64 件作品已按新评图标准完成免费重评，分数与评语均已更新。

- 这次重评不扣除账户评图额度，无需自行提交或付费。
- 作品链接、点赞和收录时间保持不变，可直接回到长廊查看更新后的结果。
- 评分更关注照片实际表达，评语围绕具体画面细节提出建议，同时保留作品已有的长处。
- 长廊翻页保持稳定顺序，避免在两页交界处重复显示同一件作品。

---

<a id="2026-09-06-image-led-critiques"></a>

## 2026-09-06 - 更懂照片表达的评图

日期：2026-09-06

### 这次更新

评图更重视画面实际表达，让不同风格的照片得到更贴合自身特点的反馈。

- 剪影、留白、黑白和安静的画面，会结合其实际效果评价，不再一律要求明亮、锐利或鲜艳。
- 优点、问题和建议围绕同一组画面细节展开，建议更注重保留照片已有的长处。
- 评语生成中断后会保留已完成的评分，重试时保持分数一致；失败不会扣除评图额度。
- 长廊旧作品暂时保留原结果，免费统一重评将在真实链路验证通过后进行，完成后另行公告。
...
这次更新根据彼此独立的传统 SEO 与 GEO 审计，修复了会影响搜索抓取、SERP 呈现、AI 引用、页面实体识别和内容可信度的跨层问题，并用真实 production server 与 50 页爬取锁定 metadata、H1、路由、Schema、缓存和 sitemap 行为。

- 未知一级路径、非法 Blog/Prompt slug 和静态页非法子路径现在会在流式渲染前返回真实 `404`，不再生成 `200` 软 404。
- 页面 title 不再重复追加品牌，英文 Blog 使用简洁 SEO title/description，`/generate` 与公开 Demo 保持单一服务端 H1。
- Retake Coach 现在是具有 canonical、sitemap、public cache 与页面 Schema 的可索引产品页；旧 Demo Review ID 永久跳转到唯一 canonical 示例；AI Markdown mirrors 则以 `noindex, follow` 和 canonical HTTP Link 避免传统搜索重复收录。
...
- 根 layout 只统一追加一次 `PicSpeak` 品牌；首页、Affiliate、Gallery、Review、Prompt 与 Lens Notes metadata 删除重复品牌和过长搜索标题。
- 7 篇英文 Lens Notes 可为搜索结果使用独立简洁 title；过长 description 与 Retake description 已收短，而可见文章标题和正文保持自然表达。
- `/generate` 的隐藏 SEO heading 改为 H2，公开 Demo 由服务器输出说明性 H1；Review 客户端结果标题按公开/私有边界选择 H2 或 H1。
- Header、Marketing Header、Footer 使用当前 locale 首页 canonical；Footer 全站增加本地化 Updates 链接，Gallery 增加指向完整公开点评示例的可爬取链接。
- 91 个 sitemap URL 中抽取 50 页进行最终爬取：全部为 200、indexable、public cache、单一 H1，title/description/canonical/OG/Twitter/lang 完整，0 个重复 title/description，0 个缺失 alt。
...
- Blog index、Gallery、Demo Review 与 Prompt 统一关联稳定 Organization/WebSite/Collection ID；AI Create 删除页面未展示的付费 Offer，只保留可见且可证明的免费 Offer。
...
- `frontend/src/components/{gallery,generation,home,layout,marketing}/`
- `frontend/src/lib/{app-route-roots,home-structured-data,seo,site,news-sitemap,blog-data,blog-references,ai-markdown,llms,demo-review,gallery-schema,gallery-seo-copy}.ts`
...
- `cd frontend && npm test` 通过，142 / 142 tests passed。
- `cd frontend && npm run build` 通过，116 个页面完成 production build 页面数据与静态生成。
- `cd frontend && npm run test:production-blog` 通过，11 / 11 production HTTP scenarios passed。
- 最终 production crawl 从 91 个 sitemap URL 抽取 50 页，传统 SEO 契约 50 / 50 通过，并成功解析 151 个 JSON-LD 节点。
...
- 生产回归测试为干净 CI 环境注入安全的本地 API 占位地址，并在服务端错误时优先输出进程日志，避免依赖开发机 `.env.local` 或被超长错误页掩盖根因。
