...
import { buildLatestRetakeChainSnapshot } from '@/lib/retake-progress';
...
function getCopy(locale: 'zh' | 'en' | 'ja') {
  if (locale === 'ja') {
    return {
      ...
      scoreVersion: '基準', excluded: (count: number) => `別基準の比較 ${count} 件は接続していません`,
  ...
  if (locale === 'en') {
    return {
      ...
      scoreVersion: 'Rubric', excluded: (count: number) => `${count} comparisons from another rubric were left unconnected`,
  ...
  return {
    ...
    scoreVersion: '标尺', excluded: (count: number) => `${count} 条其他标尺的比较未连接`,
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
            <span className="rounded-full border border-border-subtle bg-raised/70 px-3 py-1 text-ink-muted">
              {copy.scoreVersion}: {getScoreVersionLabel(snapshot.scoreVersion, locale)}
            </span>
            {snapshot.excludedVersionCount > 0 && (
              <span className="rounded-full border border-gold/25 bg-gold/10 px-3 py-1 text-gold">
                {copy.excluded(snapshot.excludedVersionCount)}
              </span>
            )}
          </div>
        </div>
