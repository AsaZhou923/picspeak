...
export function RetakeWorkspaceIntro({
  ...
  return (
    <section className="ui-feature-panel overflow-hidden border-sage/25">
      <div className="border-b border-border-subtle px-5 py-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Link href="/retake" className="min-h-11 rounded-control px-3 py-2 text-xs font-bold text-ink-muted transition-colors hover:bg-raised hover:text-gold">
            {copy.changeSource}
          </Link>
        </div>
      </div>
      ...
      {sourceLoading ? (
        ...
        <div className="flex flex-col items-center px-6 py-10 text-center">
          ...
          <Link href="/retake" className="ui-action-secondary mt-5 min-h-11 border-rust/30 px-4 py-2 text-sm text-rust hover:bg-rust/10">
            {copy.changeSource}
          </Link>
        </div>
      ) : (
        <div className="grid gap-0 sm:grid-cols-[minmax(0,1fr)_220px]">
          <div className="relative aspect-[16/9] min-h-56 overflow-hidden bg-void/40 sm:aspect-auto">
            ...
            <span className="absolute bottom-3 left-3 inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/60 px-3 py-1.5 text-[11px] text-white backdrop-blur-md">
              <ImageIcon size={12} />
              Original
            </span>
          </div>
          <div className="flex flex-col justify-center border-t border-border-subtle p-5 sm:border-l sm:border-t-0">
            <span className="ui-eyebrow text-[10px]">02 / Retake</span>
