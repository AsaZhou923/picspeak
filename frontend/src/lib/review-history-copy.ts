...
const COPY: Record<ReviewHistoryLocale, LocaleCopy> = {
  zh: {
    ...
    growth: {
      label: '近期作品表现',
      title: '按已加载点评查看作品记录',
      body: '这里汇总最近样本的评分和反复偏弱的维度，只代表这些作品在当前标尺下的表现，不等于已验证能力提升。',
      scoreVersionLabel: '当前标尺',
      mixedVersionNotice: (count: number) => `已排除 ${count} 条不同或未知标尺的记录`,
      recentAverage: '最近 3 条样本均分',
      previousAverage: '再前 3 条样本均分',
      recentList: '最近 3 次点评',
      commonGaps: '反复掉分的维度',
      trendUp: '近期样本更高',
      trendDown: '近期样本更低',
  ...
  en: {
    ...
    growth: {
      label: 'Recent Work Performance',
      title: 'Read the loaded critique records as a sample',
      body: 'This summarizes the recent scored works and repeated weak dimensions under the current rubric. It is not a verified skill-growth claim.',
      ...
      recentAverage: 'Recent 3 sample avg',
      previousAverage: 'Previous 3 sample avg',
      recentList: 'Most recent three critiques',
      commonGaps: 'Dimensions that keep dragging',
      trendUp: 'Recent sample is higher',
      trendDown: 'Recent sample is lower',
      trendFlat: 'Recent sample is similar',
      averageLabel: 'Avg',
  ...
  ja: {
    ...
    growth: {
      label: '最近の作品パフォーマンス',
      title: '読み込んだ講評をサンプルとして見る',
      body: '現在の基準で最近の作品スコアと弱い次元を整理します。検証済みの技能向上を示すものではありません。',
      scoreVersionLabel: '現在の基準',
      mixedVersionNotice: (count: number) => `異なる基準または不明な基準の記録を ${count} 件除外しました`,
      recentAverage: '直近 3 件の平均',
      previousAverage: 'その前 3 件の平均',
