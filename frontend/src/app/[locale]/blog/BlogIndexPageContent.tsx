...
function resolveBlogLocale(locale?: string): Locale {
...
export default function BlogIndexPageContent({ locale }: { locale?: string }) {
  ...
  const collectionJsonLd = {
    ...
    hasPart: posts.map((post) => ({
      '@type': 'BlogPosting',
      headline: post.title,
      url: `${siteConfig.url}/${pinnedLocale}/blog/${post.slug}`,
                    ...
                    <div className="px-6 py-7 lg:px-8">
                      ...
                      <div className="mt-6 flex flex-wrap gap-3 text-xs text-ink-subtle">
                        ...
                        <BlogViewCount
                          ...
                          className="inline-flex items-center gap-2 rounded-full border border-border-subtle px-3 py-1"
                        />
                      </div>
              ...
              <div className="grid gap-5 lg:grid-cols-3">
                {posts.map((post) => (
                  <article key={post.slug} className="group rounded-[26px] border border-border-subtle bg-raised/45 p-5 transition-all duration-300 hover:border-gold/30 hover:bg-raised/60">
                    ...
                    <div className="mt-6">
                      <Link href={`/${pinnedLocale}/blog/${post.slug}`} className="inline-flex items-center gap-2 text-sm text-gold transition-colors group-hover:text-gold-light">
                        {ui.readMoreCta}
